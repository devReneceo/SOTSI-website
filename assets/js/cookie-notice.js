/* SOTSI · Cookie notice v1.0.0 (prototipo GH Pages — espejo de sotsicookies
   1.0.0 en Webflow). Paridad con el aviso del WordPress vivo ("This website
   uses cookies… ACCEPT / Learn more") re-estilado a la marca: card navy glass,
   texto marfil, ACCEPT dorado, "Learn more" → privacy-policy/#sec-6 (sección
   "Cookies"). El link se resuelve desde el src de este script, así el mismo
   archivo sirve a cualquier profundidad (raíz, ../, ../../).

   - Aviso informativo (igual que el WP actual), no un CMP: no hay analytics y
     los embeds de YouTube usan facade click-to-play.
   - Aceptación en localStorage bajo clave VERSIONADA (sotsi_cc_v1).
   - Para re-mostrarlo en pruebas: ?cookies=reset o borrar Site Data.
   - 0 CLS (fixed), transform/opacity, prefers-reduced-motion, aria. */
(() => {
  "use strict";

  const KEY = "sotsi_cc_v1";
  const src = (document.currentScript && document.currentScript.src) || "";
  const ROOT = src.replace(/assets\/js\/cookie-notice\.js.*$/, "");
  const PRIVACY_HREF = ROOT + "privacy-policy/#sec-6";
  const COPY = "This website uses cookies to improve your experience. " +
  ...
  const show = () => {
    ...
    accept.addEventListener("click", () => {
      ...
      const gone = () => { if (bar.parentNode) bar.parentNode.removeChild(bar); };
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", () => setTimeout(show, 600), { once: true });
  } else {
    setTimeout(show, 600);
  }
})();
